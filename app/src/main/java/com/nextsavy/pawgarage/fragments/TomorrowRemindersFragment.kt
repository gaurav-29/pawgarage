...
import com.nextsavy.pawgarage.databinding.FragmentTomorrowRemindersBinding
...
class TomorrowRemindersFragment : Fragment(), RecyclerViewMenuInterface<ReminderDTO> {

    private lateinit var binding: FragmentTomorrowRemindersBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentTomorrowRemindersBinding.inflate(inflater, container, false)
    ...
    private fun initiateRemindersData() {
        reachedEnd = false
        lastDocument = null
        binding.tomorrowRemindersRV.layoutManager = LinearLayoutManager(requireContext())
        adapter = RemindersGeneralAdapter(arrayListOf(), userType, this)
        binding.tomorrowRemindersRV.adapter = adapter
    }
    private fun getTomorrowReminders() {
        if (Helper.isInternetAvailable(requireContext())) {
            ...
            val todaysDateAtTime00 = calendar.time

            // Get tomorrow's date with time 00:00:00
            calendar.time = todaysDateAtTime00
            calendar.add(Calendar.DAY_OF_MONTH, 1) //Adds a day
            val tomorrowsDateAtTime00 = calendar.time

            // Get day after tomorrow's date with time 00:00:00
            calendar.time = todaysDateAtTime00
            calendar.add(Calendar.DAY_OF_MONTH, 2) //Adds a day
            val dayAfterTomorrowsDateAtTime00 = calendar.time
            ...
            if (!reachedEnd) {
                ...
                val query = if (lastDocument == null) {
                    Firebase.firestore.collection(CollectionReminders.name)
                        .whereGreaterThanOrEqualTo(CollectionReminders.kReminderDate, tomorrowsDateAtTime00)
                        .whereLessThan(CollectionReminders.kReminderDate, dayAfterTomorrowsDateAtTime00)
                ...
                query.get().addOnSuccessListener { querySnapshot ->
                    ...
                    getAnimalProfilesFrom(distinctAnimalIdList) { pair: Pair<Exception?, List<AnimalDTO>?> ->
                        ...
                        Log.e("NST-M", "Exception: TomorrowRemindersFragment > ${qException.localizedMessage}")
