...
import com.nextsavy.pawgarage.databinding.FragmentTodayRemindersBinding
...
class TodayRemindersFragment : Fragment(), RecyclerViewMenuInterface<ReminderDTO> {

    private lateinit var binding: FragmentTodayRemindersBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentTodayRemindersBinding.inflate(inflater, container, false)
    ...
    private fun initiateRemindersData() {
        reachedEnd = false
        lastDocument = null
        binding.todayRemindersRV.layoutManager = LinearLayoutManager(requireContext())
        adapter = RemindersGeneralAdapter(arrayListOf(), userType, this)
        binding.todayRemindersRV.adapter = adapter
    }
            ...
            if (!reachedEnd) {
                ...
                val query = if (lastDocument == null) {
                    Firebase.firestore.collection(CollectionReminders.name)
                        .whereGreaterThanOrEqualTo(CollectionReminders.kReminderDate, todaysDateAtTime00)
                        .whereLessThan(CollectionReminders.kReminderDate, tomorrowsDateAtTime00)
                ...
                query.get().addOnSuccessListener { querySnapshot ->
                    ...
                    Log.e("NST-M", "Exception: TodayRemindersFragment > ${qException.localizedMessage}")
